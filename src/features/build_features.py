...
def _map_binary_series(s: pd.Series) -> pd.Series:
    ...
    # CRITICAL: These exact string mappings are hardcoded in serving pipeline
    ...
    ## for any other 2-category feature, we use stable alphabetical ordering
    if len(vals) == 2:
...
def build_features(df: pd.DataFrame, target_col: str = "Churn") -> pd.DataFrame:
    ...
    ## CRITICAL: drop_first=True helps prevent multicollinearity
    if multi_cols:
